...
const Sip = () => {
  ...
  const [sip_vlaue,setSipValue]=useState(0);


  const handleAddSIP = () => {

    if (name.length === 0) {
      alert("SIP Name is left");
    }
    else if (sip_vlaue === 0) {
        alert("SIP Valuation is left");
      }
    else {
      if (confirm(`Conferm to Add sip  => ${name}`)) {
        setSpinner(1);
        const url = `${process.env.REACT_APP_domain}/sjh-team-api/admin/add_sip.php`;
        let fData = new FormData();
        fData.append('sip_name', name);
        fData.append('sip_value',sip_vlaue);
        fData.append('team', localStorage.getItem('team'));
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center m-5">
      <div className="bg-white shadow-xl rounded-lg p-8 w-full max-w-md">
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Add SIP Amount</h2>
  
        {/* User Name Select Field */}
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-semibold mb-2">SIP Name</label>
          ...
          <option >Select SIP name</option>
            {data.map((item ,index) => (
              <option key={index}>{item.sip_name}</option>
            ))}
          </select>
        </div>
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-semibold mb-2">SIP Valuation</label>
          <input value={sip_vlaue} onChange={(e)=>{setSipValue(e.target.value)}}  className="block w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none" type="number" />
        </div>
      ...
      <table className='w-[100%]'>
        <thead>
          <tr className='bg-orange-500 text-white'>
            ...
            <th className='p-2 text-center'>SIP Name</th>
            <th className='p-2 text-center'>SPI Amount</th>
            <th className='p-2 text-center'>SIP invest</th>
            <th className='p-2 text-center'>SIP value</th>
          </tr>
        </thead>
        <tbody>
          {data.map((sip, index) => (
            <tr key={index} className='hover:bg-gray-100'>
              ...
              <td className='p-2 border-b border-gray-200'>{sip.sip_name}</td>
              <td className='p-2 text-right border-b border-gray-200'>{sip.sip_amt}</td>
              <td className='p-2 text-right border-b border-gray-200'>{sip.sip_invest}</td>
              <td className='p-2 text-right border-b border-gray-200'>{sip.sip_value}</td>
            </tr>
          ))}
